...
const workflow = function({base_url}) {
    return {
        query({jwt_token, meta, event, query}, cb) {
            throw("query API method is outdated");
            const config = {
                headers: {}
            };
            if(jwt_token) {
...
const rpc = function({base_url}) {
    return function({jwt_token, model, proc, args}, cb) {
            const config = {
                headers: {}
            };
            if(jwt_token) {
            ...
            return axios.post(base_url+'/rpc', {model, proc, args}, config).then((response) => {
                if(cb && response)
                    cb(response.data);
            });
    }
